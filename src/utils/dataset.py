from src.utils.lmdb import get_array_shape_from_lmdb, retrieve_row_from_lmdb
...
class TextVideoPairDataset(Dataset):
    def __init__(self, meta_data_path, transform = None, duplication=1):
        """
        Args:
            meta_data_path (str): Path to a JSON lines file where each line is a JSON object
                                  with 'video_path' and 'prompt' fields.
        """
        self.prompt_list = []
        ...
        with open(meta_data_path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                ...
                if 'prompt' not in data or 'video_path' not in data:
                    raise ValueError(f"Missing 'prompt' or 'video_path' in row {i}: {data}")
                self.prompt_list.extend([data['prompt']] * duplication)
                self.video_path_list.extend([data['video_path']] * duplication)

    def load_video_path(self, video_path, device=None, dtype=None):
    ...
    def __getitem__(self, idx):
        ...
        video = self.load_video_path(video_path)
...
class VideoDataset(Dataset):
    def __init__(self, video_dir, transform = None, duplication=1):
        """
        Args:
            video_dir (str): Path to a directory containing video files.
        """
        self.video_dir = Path(video_dir)
        self.video_path_list = sorted(list(self.video_dir.glob("*.mp4")))
        self.transform = transform
    ...
    def load_video_path(self, video_path, device=None, dtype=None):
        ...
        if self.transform:
            ...
            video = self.transform(video[0]).unsqueeze(0)
        return video
