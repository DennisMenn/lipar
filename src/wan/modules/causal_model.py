from src.wan.modules.attention import attention
from src.utils.rlt_prune import unprune_kv
from src.wan.modules.model import (
    WanRMSNorm,
    rope_apply,
    WanLayerNorm,
...
def causal_rope_apply(x, grid_sizes, freqs, start_frame=0, prune_mask=None):
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        seq_len = x[i].shape[0]
        max_seq_len = f * h * w # this is the sequence length before pruning

        assert seq_len <= max_seq_len, "seq_len must be less than or equal to f * h * w otherwise will have elements without rope"
        ...
        freqs_i = torch.cat([
            ...
            dim=-1).reshape(max_seq_len, 1, -1)
        
        if prune_mask is not None:
            freqs_i = freqs_i[prune_mask[i]]
...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            TOKENS_PER_BLOCK = TOKENS_PER_FRAME * 3 

            frame_seqlen = math.prod(grid_sizes[0][1:]).item()
            current_start_frame = current_start // frame_seqlen
            current_end = current_start + TOKENS_PER_BLOCK # end token of the input (unpruned) video frames
            sink_tokens = self.sink_size * frame_seqlen # don't prune for sink token 
            
            # Unprune KV cache
            is_memory_step = (diffusion_step.item() == 0)
            is_first_denoising = (current_start == 0)

            #Can't unprune: 1. required clean token (from_memory) 2. first denoising step 3. not a memory step 
            if from_memory and is_first_denoising and (not is_memory_step): is_unprune_kv = False
            ...
            if trim_kv is not None:
                first_frames_end = trim_kv[0]
                last_frames_start = trim_kv[1]
                first_token_end = min(first_frames_end * TOKENS_PER_FRAME, local_end_index)
                last_token_start = max(first_token_end, local_end_index - TOKENS_PER_FRAME * last_frames_start)

                sel_cache = torch.full([local_end_index], False, device=kv_cache["k"].device)
                sel_cache[:first_token_end] = True  # keep first frames
                sel_cache[last_token_start:local_end_index] = True  # keep last frames

                k_cache = kv_cache["k"][:, :local_end_index][:, sel_cache, :, :]
                v_cache = kv_cache["v"][:, :local_end_index][:, sel_cache, :, :]
            else:
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # self-attention        
        module_x = self.norm1(x) * (1 + e[1][:,0]) + e[0][:,0]
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _forward_inference(
        ...
        # obtain the mask
        assert len(x) == 1 and x[0].shape[0] == 1, "Only support batch size of 1 for inference"
        if is_prune:
            x = [u[0][prune_mask[0]].unsqueeze(0) for u in x]
        ...
        #restore shape by filling with NaN
        # Fill the output tensor with NaN at positions that were removed by prune_mask
        if is_prune:
            output = torch.full((*prune_mask.shape, x.shape[-1]), float('nan'), device=x.device, dtype=x.dtype)
            output[prune_mask] = x
            x = output
        # head
        # x.shape [1, 4680, 1536]
        ...
        # x.shape list [16, 3, 60, 104]
        return torch.stack(x) # x.shape list [1, 16, 3, 60, 104]
